...
from common.utils import PAYOFF_KIND_TO_ID, str_list_to_int
...
class Payoff:
    ...
    def tansho(self) -> pd.DataFrame:
        df = self.table[self.table['ticket_type'] == PAYOFF_KIND_TO_ID.get(
            '単勝')][['race_id', 'pattern', 'payoff', 'popularity']]
    ...
    def fukusho(self) -> pd.DataFrame:
        df = self.table[self.table['ticket_type'] == PAYOFF_KIND_TO_ID.get(
            '複勝')][['race_id', 'pattern', 'payoff', 'popularity']]
        df['pattern'] = df['pattern'].astype(int)
        return df
    ...
    def umaren(self) -> pd.DataFrame:
        df = self.table[self.table['ticket_type'] == PAYOFF_KIND_TO_ID.get(
            '馬連')][['race_id', 'pattern', 'payoff', 'popularity']]
    ...
    def sanrenfuku(self) -> pd.DataFrame:
        df = self.table[self.table['ticket_type'] == PAYOFF_KIND_TO_ID.get(
            '3連複')][['race_id', 'pattern', 'payoff', 'popularity']]
        df['pattern'] = df['pattern'].str.split('-').map(str_list_to_int)
        return df
    ...
    def wakuren(self) -> pd.DataFrame:
        df = self.table[self.table['ticket_type'] == PAYOFF_KIND_TO_ID.get(
            '枠連')][['race_id', 'pattern', 'payoff', 'popularity']]
        df['pattern'] = df['pattern'].str.split('-').map(str_list_to_int)
        return df
    ...
    def wide(self) -> pd.DataFrame:
        df = self.table[self.table['ticket_type'] == PAYOFF_KIND_TO_ID.get(
            'ワイド')][['race_id', 'pattern', 'payoff', 'popularity']]
        df['pattern'] = df['pattern'].str.split('-').map(str_list_to_int)
        return df
    ...
    def umatan(self) -> pd.DataFrame:
        df = self.table[self.table['ticket_type'] == PAYOFF_KIND_TO_ID.get(
            '馬単')][['race_id', 'pattern', 'payoff', 'popularity']]
        df['pattern'] = df['pattern'].str.split('→').map(str_list_to_int)
        return df
    ...
    def sanrentan(self) -> pd.DataFrame:
        df = self.table[self.table['ticket_type'] == PAYOFF_KIND_TO_ID.get(
            '3連単')][['race_id', 'pattern', 'payoff', 'popularity']]
        df['pattern'] = df['pattern'].str.split('→').map(str_list_to_int)
        return df
